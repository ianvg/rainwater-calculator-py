...
The **Design recommendations** panel above the table provides several decision aids rather than one universal answer. Set the reliability target and the diminishing-return threshold, measured as reliability percentage points gained per additional 1,000 gallons, then choose **Refresh**. The panel identifies the smallest simulated tank meeting the target, the capacity immediately before the first marginal gain at or below the threshold, the candidate with the lowest finite simple payback when economics are configured, and nearby simulated alternatives. These settings are saved with the project but do not invalidate hydraulic results because they only change how existing candidate results are interpreted.

The panel also reports review conditions for selected-tank reliability below the target, overflow above 25% of simulated tank throughput, the rainfall completeness score, the longest missing period, explicitly named partial or incomplete calendar years, unclassified rainfall, missing timezone metadata, non-daily source labels, and unconfigured financial inputs. Review conditions are prompts for engineering judgment, not automatic pass/fail criteria.
...
The dedicated **Optimization** tab includes a shared equipment library and a separate project candidate set under **Optimization problem definition and assumptions**. Apply reusable products to a project, classify them as candidate, fixed, or excluded, review project eligibility and compatibility, and use explicit library-update or project-override controls when product data changes. The starter products are illustrative and are not vendor data or market quotations. See [Engineering optimization](optimization.md) for the complete workflow and [Equipment catalog roadmap](equipment-catalog-roadmap.md) for planned rule capabilities.
...
At minimum, review **Minimum rainwater reliability**, **Electricity price**, and the selected objective before running the optimizer. The calculation reuses rainfall, collection, demand, minimum operating level, filter recovery, municipal-backup settings, and the assumptions on the **Financial analysis** tab. Catalog component costs are added to the base installed system cost. Estimated transfer-pump electricity and annual maintenance are deducted from annual water and sewer savings.
