...
Monthly values are edited directly in the demand-object dialog. Recurring daily demand can optionally use January-December daily-volume overrides; monthly-volume mode requires a total for at least one month. Its Schedule list also provides **-- No schedule --**. With no schedule, each monthly total is divided across the month's calendar days and then distributed equally over 24 hours. Selecting a schedule instead distributes the daily total across that schedule's active hours. The live summary estimates a typical weekday, typical week, January demand, and sewer-charge eligibility. Inline messages identify missing names, schedules, weekdays, nonnumeric values, and zero-demand configurations before the object can be saved.
...
Except for monthly-volume objects using **-- No schedule --**, only schedules already present in the current project's Schedules list can be assigned. Copy a template or custom library schedule into the project before creating other object modes. Each demand object has an instantaneous on-flow that can be entered in **gpm**, **gal/hr**, **lpm**, or **liter/hr**. Switching the unit in the editor converts the current value without changing the physical flow. For every hour, demand equals the on-flow multiplied by the schedule value and the appropriate time conversion. Daily analysis sums those hourly volumes; hourly analysis applies each hourly volume directly. Recurring daily objects use explicit Monday-Sunday checkboxes rather than assuming that a count of operating days always means Monday through Friday. A project schedule cannot be deleted while a demand object references it, and renaming the schedule updates those references automatically.
