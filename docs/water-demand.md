...
Open **Schedules**, immediately after **Rainwater Data**. The left management pane follows the OpenStudio toolbar pattern: select the white plus in the green circle to create and edit a typical-week schedule, select the white `x2` in the blue circle to duplicate the selected schedule, or select the white x in the red circle to delete it. The three controls are adjacent. Schedule copies are persisted with the project, and the selected list item is the active hourly profile. In **Analysis settings > Demand timing**, choose **Hourly** to apply that profile hour by hour or **Daily** to sum each day's preserved hourly values and apply the total once at 12:00 noon. Deleting the final schedule switches demand timing to Daily and restores the default even profile.
...
Use the **Active months** checkboxes under Schedule properties to choose when the selected schedule applies. A demand object assigned to that schedule contributes demand only during the checked months. New schedules and older projects default to all 12 months. Duplicating a schedule or saving it to the custom library preserves its month selection.
...
This follows the general [OpenStudio ScheduleRuleset](https://openstudio-sdk-documentation.s3.amazonaws.com/cpp/OpenStudio-3.8.0-doc/model/html/classopenstudio_1_1model_1_1_schedule_ruleset.html) pattern: time/value day profiles are selected according to the month and day of week. The calculator provides month applicability plus one explicit typical week rather than arbitrary date-range rules, holidays, or design-day schedules. Because imported ACIS and ECCC rainfall is daily, each day's collected rainfall enters after hour 23, at the end-of-day midnight boundary. It cannot satisfy demand from earlier hours on that date.
...
For an indirect system, end-use demand draws from the buffer tank. When the buffer level falls below the configured **Buffer refill level**, a refill cycle starts and remains active until the buffer tank is full. The buffer refill level must be at least its **Minimum operating level**. Water at or below that level remains physically stored but is unavailable to normal end-use demand, just as the primary-tank reserve is unavailable to demand and transfer-pump withdrawals. Filtration can use Infinite flow or 15, 20, 30, 40, or 50 GPM skids. Multiple systems in parallel add their nominal capacities, and the transfer pump follows that combined flow; the default is one 20 GPM system. Infinite flow lets the requested end-use or buffer-refill flow pass without a filtration-capacity limit. Filter recovery is applied to pumped flow. If primary-tank water cannot meet the commanded refill flow and municipal backup is enabled, unlimited-capacity municipal water supplies the difference directly to the buffer tank. The simulation tracks municipal and rainwater volumes separately so municipal makeup does not count as rainwater reliability. Rainfall enters the primary tank at the end of each simulated day.

Hourly results include primary-tank and buffer-tank total capacity, minimum operating volume, usable capacity, physical level, usable water available, pump delivery, filter throughput and loss where applicable, municipal makeup, rainwater shortfall attributable to operating reserves, system unmet demand, and overflow. Distribution-pump capacity, filtration-system flow, transfer-pump type, filtration recovery, buffer storage, initial buffer fill, refill level, minimum operating level, and municipal backup availability are configured under **System parameters**. A zero distribution-pump capacity means unlimited direct-system flow; a zero buffer size keeps the indirect path in pass-through mode without refill control.
