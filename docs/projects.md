...
The **System builder** provides a drag-and-drop workspace with a right-side notebook. The **System object library** tab contains rainwater input, primary tank, filtration pump, filtration system, booster tank, booster pump, municipal water backup, end-uses, and overflow discharge. Double-click an object, press Enter, use **Add selected**, or drag it from the library onto the canvas. A preview follows the pointer during a library drag. The **Edit** tab edits the selected placed object; its first available property is **Name**, which updates the canvas label and is saved with the project. Drag placed objects to reposition them. Each object exposes circular inlet and outlet nodes where applicable. Select an outlet node and then an inlet node to create a directional link; Escape cancels an unfinished link. Select a component or link and use Delete or **Delete selected** to remove it. Component positions and links are saved with the project.

Selecting **Custom system** makes the saved topology drive the hourly hydraulic simulation. A valid custom system has exactly one rainwater input connected directly to one primary tank, one primary-tank overflow discharge path, and one ordered service path from the primary tank to end-uses. The service path can include a filtration pump, filtration system, booster tank, and booster pump in that order. Municipal backup is optional and connects to either the booster tank or end-uses. Cycles, disconnected service components, incompatible ordering, and unsupported branching are rejected before the custom system is applied or simulated. Pump limits, filter recovery, storage levels, municipal supply, overflow, unmet demand, and losses use the component parameters above. Hourly results include a mass-balance residual; a valid simulation conserves flow to numerical precision. Direct and indirect modes continue to use their predefined templates. The previous indirect-system schematic is retained at `assets/indirect_system.svg`.
