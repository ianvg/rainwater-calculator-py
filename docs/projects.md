...
The **System builder** provides a drag-and-drop workspace with a right-side notebook. The **System object library** tab contains rainwater input, primary tank, filtration pump, filtration system, booster tank, booster pump, municipal water backup, and end-uses. Double-click an object, press Enter, use **Add selected**, or drag it from the library onto the canvas. The **Edit** tab edits the selected placed object; its first available property is **Name**, which updates the canvas label and is saved with the project. Selecting an **End-uses** object also shows available and assigned project demand objects. Use **Add selected** or **Remove selected** (or double-click a list item) to manage its assignments; the canvas object shows the assigned demand count. Assignments are saved with the project and remain synchronized when a demand object is deleted. Drag placed objects to reposition them. Each object exposes circular inlet and outlet nodes where applicable. Select an outlet node and then an inlet node to create a directional link; Escape cancels an unfinished link. Select a component or link and use Delete or **Delete selected** to remove it. Component positions and links are saved with the project. The current simulation still follows the selected direct or indirect system template and component parameters; topology-driven simulation is planned next. The previous indirect-system schematic is retained at `assets/indirect_system.svg`.
