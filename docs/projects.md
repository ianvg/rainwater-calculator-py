...
The **System builder** provides a drag-and-drop workspace with a system-object library. Initial objects include rainwater input, primary tank, filtration pump, filtration system, booster tank, booster pump, municipal water backup, and end-uses. Double-click an object, press Enter, use **Add selected**, or drag it from the library onto the canvas. Drag placed objects to reposition them and use Delete or **Delete selected** to remove the active object. Component positions are saved with the project. The current simulation still follows the selected direct or indirect system template and component parameters; user-defined connections and topology-driven simulation are planned next. The previous indirect-system schematic is retained at `assets/indirect_system.svg`.
