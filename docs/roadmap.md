...
[Implemented] The separate **Multi-project comparison report** lets users select projects from the current database and add projects from other rainwater project database files without changing or overwriting the project currently open in the desktop application. External databases are schema-checked and opened through a query-only SQLite connection.

[Implemented initial comparison set] The report normalizes displayed units and compares location and rainfall provenance, collection area, average annual precipitation, area-weighted runoff coefficient, demand, selected tank size, annual rainwater supplied, reliability, municipal makeup, system unmet demand, overflow, first-flush and treatment losses, installed cost, net annual savings, simple payback, and major review conditions. Missing, stale, incompatible, and never-run analysis results are identified rather than silently recalculated or treated as zero.

[Implemented for HTML and direct PDF] A versioned, validated comparison model feeds portable HTML and direct-PDF renderers. The HTML output includes an accessible reliability chart, corresponding data tables, clearly labelled project-specific assumptions, and review conditions. Tests cover unit normalization, different rainfall periods, duplicate names across sources, missing or incompatible files, read-only access, and partial results. LaTeX comparison output and broader lifecycle-financial parity remain future extensions.
