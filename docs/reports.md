...
The tank summary appears below the surface-area summary. For each configured primary or buffer tank it reports total physical capacity, minimum operating volume, usable capacity, final physical storage, and final usable water. Overflow is based on total physical capacity, while water at or below the minimum operating level is unavailable to normal withdrawals. The Executive Summary and candidate table also report reserve-caused rainwater shortfall, calculated against a stateful simulation of the same inputs with both operating levels set to zero.
