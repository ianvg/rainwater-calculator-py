...
The **Antecedent dry period** defines rainfall events using rainfall history and defaults to the paper's recommended single dry day. A wet observation begins a new event when it is the first wet observation or when the time since the preceding wet observation exceeds the configured number of days. First flush is diverted only on that first wet observation; consecutive wet days do not divert another first flush. If rainfall on the first day is shallower than the configured diversion depth, all runoff from that day is diverted and no unused allowance carries to the next day.
